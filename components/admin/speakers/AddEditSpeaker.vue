    ...
    <v-card rounded="xl" class="pa-4">
      <v-container fluid>
        <v-row>
          <v-col>
            <h4>
              {{ props.type === "add" ? "Add Speaker" : "Edit Speaker" }}
            </h4>
          </v-col>
        </v-row>
        <v-row class="my-5">
...
const { addSpeaker, updateSpeaker } = useSpeakers();
...
const addSpeakerData = async () => {
  try {
    loading.value = true;
    let res = await addSpeaker("speakers", formData.value);
