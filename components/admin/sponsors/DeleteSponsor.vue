  ...
  <v-dialog v-model="dialog" width="500">
    ...
    <v-card rounded="xl" class="pa-4">
      <v-container fluid>
        <v-row>
          <v-col>
            <h4>Delete Sponsor</h4>
          </v-col>
        </v-row>
        <v-row class="mt-5">
          <v-col md="12" cols="12" class="my-n2">
            <p>Are you sure?</p>
            <p>Deleted Sponsor Can't be Recover!!</p>
          </v-col>
          <v-col md="12" cols="12" class="mt-4">
            <v-btn
              :loading="loading"
              @click="deleteSponsorData"
              variant="flat"
              color="primary"
              >Delete</v-btn
            >
...
const deleteSponsorData = async () => {
  try {
    loading.value = true;
    console.log("deleting tEAM whose doc id is::::", props.docid);
    let res = await deleteSponsor("sponsors", props.docid);
